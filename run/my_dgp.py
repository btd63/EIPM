...
def save(payload: Dict[str, Any], out_name: str, save_csv: bool = True) -> Tuple[str]: # payload를 out_name의 이름으로 저장
    ...
    tmp_name = npz_name + ".tmp.npz"
    np.savez_compressed(tmp_name, **payload)
    os.replace(tmp_name, npz_name)
    ...
    if save_csv:
        ...
        if isinstance(X_all, np.ndarray) and isinstance(T_all, np.ndarray) and isinstance(Y_all, np.ndarray):
            _ensure_csv_complete(out_dir, out_name, X_all, T_all, Y_all)
    return npz_name


def _csv_rep_is_valid(rep_path: str, n_train: int, n_cols: int) -> bool:
    try:
        if not os.path.exists(rep_path):
            return False
        with open(rep_path, "r", encoding="utf-8") as f:
            lines = f.read().splitlines()
        if len(lines) != int(n_train) + 1:
            return False
        if len(lines[0].split(",")) != int(n_cols):
            return False
        if int(n_train) > 0 and len(lines[1].split(",")) != int(n_cols):
            return False
        return True
    except Exception:
        return False
...
def _write_rep_csv_atomic(rep_path: str, rows: np.ndarray, header: str) -> None:
    tmp_path = rep_path + ".tmp"
    np.savetxt(tmp_path, rows, delimiter=",", header=header, comments="")
    os.replace(tmp_path, rep_path)


def _ensure_csv_complete(out_dir: str, out_name: str, X_all: np.ndarray, T_all: np.ndarray, Y_all: np.ndarray) -> Tuple[int, int]:
    csv_dir = os.path.join(out_dir, "csv", out_name)
    os.makedirs(csv_dir, exist_ok=True)
    n_rpt = int(X_all.shape[0])
    n_train = int(X_all.shape[1])
    d_x = int(X_all.shape[2])
    n_cols = d_x + 2
    ...
    for r in range(n_rpt):
        ...
        if _csv_rep_is_valid(rep_path, n_train=n_train, n_cols=n_cols):
            kept += 1
            continue
        Xr = X_all[r]
        ...
        _write_rep_csv_atomic(rep_path, rows, header=header)
        fixed += 1
    return kept, fixed


def _is_valid_existing_npz(npz_path: str, cfg: "Config") -> Tuple[bool, str]:
    required = ["X_train", "T_train", "Y_train", "T_eval", "mu_eval", "n_rpt", "n_train", "n_eval", "d_X"]
    try:
        with np.load(npz_path, allow_pickle=True) as data:
            ...
            n_rpt = int(np.asarray(data["n_rpt"]).reshape(-1)[0])
            n_train = int(np.asarray(data["n_train"]).reshape(-1)[0])
            n_eval = int(np.asarray(data["n_eval"]).reshape(-1)[0])
            d_x = int(np.asarray(data["d_X"]).reshape(-1)[0])
            if (n_rpt != int(cfg.n_rpt)) or (n_train != int(cfg.n_train)) or (n_eval != int(cfg.n_eval)) or (d_x != int(cfg.d_X)):
                return (
                    False,
                    f"meta mismatch: got (n_rpt={n_rpt}, n_train={n_train}, n_eval={n_eval}, d_X={d_x}) "
                    f"expected ({cfg.n_rpt}, {cfg.n_train}, {cfg.n_eval}, {cfg.d_X})",
                )

            x_shape = tuple(np.asarray(data["X_train"]).shape)
            t_shape = tuple(np.asarray(data["T_train"]).shape)
            y_shape = tuple(np.asarray(data["Y_train"]).shape)
            te_shape = tuple(np.asarray(data["T_eval"]).shape)
            mu_shape = tuple(np.asarray(data["mu_eval"]).shape)

            if x_shape != (cfg.n_rpt, cfg.n_train, cfg.d_X):
                return False, f"X_train shape mismatch: {x_shape}"
            if t_shape != (cfg.n_rpt, cfg.n_train):
                return False, f"T_train shape mismatch: {t_shape}"
            if y_shape != (cfg.n_rpt, cfg.n_train):
                return False, f"Y_train shape mismatch: {y_shape}"
            if te_shape != (cfg.n_rpt, cfg.n_eval):
                return False, f"T_eval shape mismatch: {te_shape}"
            if mu_shape != (cfg.n_rpt, cfg.n_eval):
                return False, f"mu_eval shape mismatch: {mu_shape}"
    except Exception as e:
        return False, f"npz load/parse error: {e}"
    return True, "ok"
...
def Z_to_X(Z: np.ndarray) -> np.ndarray: # Z를 X로 변환.
    ...
    for j in range(n_blocks):
        ...
        X[:, base + 3] = (z2 + z4 + 5.0) ** 3
        X[:, base + 4] = (z3 + 0.5 * (z3 ** 3)) + z5 * np.exp(z3 / 2.0)
    return X
...
def moments_X(d_X: int) -> Tuple[np.ndarray, np.ndarray]: # EX, VX 계산 코드
    ...
    # (4) X4 = (Z2+Z4+5)^3 where Z2+Z4 ~ N(0,2)
    ex4 = 155.0
    vx4 = 14970.0

    # (5) X5 = (Z3 + 0.5*Z3^3) + exp(Z3/2)*Z5 where (Z3,Z5) i.i.d. N(0,1)
    ex5 = 0.0
    vx5 = 7.75 + np.exp(0.5)
...
def generate_one_replication(
    ...
    # shrink treatment-effect parameters to improve overlap
    beta_T0 = beta_T0 * cfg.beta_T0_scale
    beta_T = beta_T * cfg.beta_T_scale
    T_train = generate_T(
        rng=rng,
...
def generate_dataset(cfg: Config) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        ...
        "beta_T0_scale": np.array(cfg.beta_T0_scale),
        "beta_T_scale": np.array(cfg.beta_T_scale),
...
def main() -> None:
    ...
    if treatment_k > args.d_X:
        raise ValueError(f"treatment_k ({treatment_k}) must be <= d_X ({args.d_X})")
    if outcome_k > args.d_X:
        raise ValueError(f"outcome_k ({outcome_k}) must be <= d_X ({args.d_X})")
    ...
    def _fmt_scale(x: float) -> str:
        s = f"{x:.3g}"
        return s.replace(".", "p")
    if abs(args.beta_T0_scale - 1.0) < 1e-12 and abs(args.beta_T_scale - 1.0) < 1e-12:
        out_name = base_name
    else:
        out_name = base_name + f"_bT0s{_fmt_scale(args.beta_T0_scale)}_bTs{_fmt_scale(args.beta_T_scale)}"
    ...
    cfg = Config(
        ...
        outcome_k=outcome_k,
        beta_T0_scale=args.beta_T0_scale,
        beta_T_scale=args.beta_T_scale,
    )

    npz_path = os.path.join(DATA_DIR, f"{out_name}.npz")
    if args.skip_if_exists and os.path.exists(npz_path):
        ok, msg = _is_valid_existing_npz(npz_path, cfg)
        if ok:
            if bool(args.save_csv):
                with np.load(npz_path, allow_pickle=True) as data:
                    X_all = np.asarray(data["X_train"])
                    T_all = np.asarray(data["T_train"])
                    Y_all = np.asarray(data["Y_train"])
                kept, fixed = _ensure_csv_complete(DATA_DIR, out_name, X_all, T_all, Y_all)
                print(f"[SKIP] Valid existing dataset: {npz_path} | csv kept={kept} fixed={fixed}")
            else:
                print(f"[SKIP] Valid existing dataset: {npz_path}")
            return
        print(f"[WARN] Existing dataset is invalid; regenerating: {npz_path}")
        print(f"[WARN] Reason: {msg}")
