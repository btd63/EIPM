...
def estimate_adrf_local_poly(
    ...
    alpha: float,
    degree: int = 1,
    logw_at_t: Callable[[float], np.ndarray] | None = None,
) -> np.ndarray:
    preds = []
    T_obs = np.asarray(T_obs, dtype=np.float64).reshape(-1)
    Y_obs = np.asarray(Y_obs, dtype=np.float64).reshape(-1)
    log_weights_arr = None
    ...
    for t_val in t_grid:
        t0 = float(t_val)
        if logw_at_t is not None:
            logw_cur = np.asarray(logw_at_t(t0), dtype=np.float64).reshape(-1)
        else:
            if log_weights_arr is None:
                raise ValueError("Either log_weights or logw_at_t must be provided.")
            logw_cur = log_weights_arr
        diff = T_obs - t0
        dist = np.abs(diff)
        ...
        if int(degree) == 0:
            ...
            if np.isfinite(s) and s > 0.0:
                W = W / s
            preds.append(float(np.sum(W * Y_obs)))
        else:
            S0 = float(np.sum(W))
            S1 = float(np.sum(W * diff))
            S2 = float(np.sum(W * diff * diff))
            T0 = float(np.sum(W * Y_obs))
            T1 = float(np.sum(W * diff * Y_obs))
            den = S0 * S2 - S1 * S1
            preds.append(float((S2 * T0 - S1 * T1) / (den + 1e-12)))
...
def estimate_adrf_local_poly_knn(
    ...
    for t0, h_t in zip(t_grid, h_all):
        if logw_at_t is not None:
            logw_cur = np.asarray(logw_at_t(float(t0)), dtype=np.float64).reshape(-1)
        else:
            if log_weights_arr is None:
                raise ValueError("Either log_weights or logw_at_t must be provided.")
            logw_cur = log_weights_arr
...
def main() -> None:
    ...
    for rep_idx in range(n_reps):
        ...
        def _logw_at_t_scaled(t_scaled_val: float) -> np.ndarray:
            t_fixed = torch.full_like(T_scaled, fill_value=float(t_scaled_val))
            with torch.no_grad():
                return model(X_scaled, t_fixed).detach().cpu().numpy().reshape(-1)
        ...
        if args.top_k_errors and int(args.top_k_errors) > 0:
            ...
            if args.debug_topk:
                ...
                for i in idx:
                    logw_i = _logw_at_t_scaled(float(T_eval_scaled[i]))
                    if h_type == "knn":
                        dbg = _local_linear_debug_knn(
                            T_obs=np.asarray(T_obs_scaled, dtype=np.float64),
                            Y_obs=np.asarray(rep.Y, dtype=np.float64),
                            logw=np.asarray(logw_i, dtype=np.float64),
    ...
    rmse_mean = float(np.sqrt(mse_mean)) if np.isfinite(mse_mean) else float("nan")
    ...
    print(
        f"[SUMMARY] MSE_mean={mse_mean:.6g} MAE_mean={mae_mean:.6g} "
        f"BIAS_mean={bias_mean:.6g} RMSE_mean={rmse_mean:.6g}"
    )
