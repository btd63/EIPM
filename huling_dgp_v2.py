#!/usr/bin/env python3
"""
Huling-style DGP with log(A)|X ~ Normal (log-normal treatment).
...
def simulate_one_rep(
    ...
    # log-normal treatment with E[A|X] ≈ muA + offset
    muA = 5.0 * np.abs(X1) + 6.0 * np.abs(X2) + 3.0 * np.abs(X5) + np.abs(X4)
    muA = np.maximum(muA + float(offset), 1e-8)
    sigma = float(sigma_logA)
    meanlog = np.log(muA) - 0.5 * (sigma ** 2)
    A = np.exp(meanlog + sigma * rng.normal(loc=0.0, scale=1.0, size=nobs))
    ...
    if eval_mode == "grid":
        ...
        muA_eval = np.maximum(muA_eval + float(offset), 1e-8)
        meanlog_eval = np.log(muA_eval) - 0.5 * (sigma ** 2)
        t_eval = np.exp(meanlog_eval + sigma * rng.normal(loc=0.0, scale=1.0, size=int(n_eval)))
        mu_eval = true_adrf(t_eval)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--sigma_logA", type=float, default=0.5)
...
def main() -> None:
    ...
    out_name = (
        f"sim_huling_logA_ntr{args.n_train}_nev{args.n_eval}_rpt{args.n_rpt}"
        f"_mx1{args.MX1}_mx2{args.MX2}_mx3{args.MX3}"
        f"_ae{args.A_effect}_sig{args.sigma_logA}_off{args.offset}_seed{args.seed}"
    )
    ...
    for r in range(int(args.n_rpt)):
        ...
        rep = simulate_one_rep(
            ...
            sigma_logA=float(args.sigma_logA),
            offset=float(args.offset),
        )
        reps.append(rep)

    payload: Dict[str, Any] = {
        "scenario": np.array("huling_logA"),
        "d_X": np.array(5),
        ...
        "sigma_logA": np.array(float(args.sigma_logA)),
        "offset": np.array(float(args.offset)),
    }
